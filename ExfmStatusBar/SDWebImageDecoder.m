...
- (void)notifyDelegateOnMainThreadWithInfo:(NSDictionary *)dict
{
    ...
    NSImage *decodedImage = [dict objectForKey:DECOMPRESSED_IMAGE_KEY];
...
{
    NSImage *image = [decodeInfo objectForKey:IMAGE_KEY];

    NSImage *decompressedImage = [NSImage decodedImageWithImage:image];
...
- (void)decodeImage:(NSImage *)image withDelegate:(id<SDWebImageDecoderDelegate>)delegate userInfo:(NSDictionary *)info
{
...
@implementation NSImage (ForceDecode)

+ (NSImage *)decodedImageWithImage:(NSImage *)image
{
    ...
    NSImage *decompressedImage = [[NSImage alloc] initWithCGImage:decompressedImageRef size:image.size];
